	...
	while true do
		...
		if uberBasladi and not paketNoktasi then
			...
			if uberBasladi then
				...
				ESX.ShowNotification(_U('uber_new_delivery') .. Config.uberItems[uberChoosenItem]["label"] .. _U('uber_marked'))
				paketNoktasi = true
                ...
                SendNUIMessage({event = 'updateSiparisUrun', urun = Config.uberItems[uberChoosenItem]["label"]})
				blipOlustur()
			end
