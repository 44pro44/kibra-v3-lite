...
function YellowPagesPostIlan(a, b, c, d, e)
    getUserYellow(d, function(f)
        exports['ghmattimysql']:execute("SELECT phone_number FROM users WHERE users.identifier = @realUser", {["@realUser"] = d}, function(g)exports['ghmattimysql']:execute("INSERT INTO yellowpages_posts (`authorId`, `message`, `image`, `realUser`, `phone`) VALUES(@authorId, @message, @image, @realUser, @phone);", {["@authorId"] = f.id, ["@message"] = a, ["@image"] = b, ["@realUser"] = d, ["@phone"] = g[1].phone_number}, function(h)
            post = {}
            post["authorId"] = f.id;
            post["message"] = a;
            post["image"] = b;
...
AddEventHandler('gcPhone:yellow_postIlan', function(message, image)
    ...
    local maven_datastore = exports.ghmattimysql:executeSync("SELECT username FROM twitter_accounts WHERE identifier = @identifier", { ['@identifier'] = xPlayer.identifier })
    for _, i in ipairs(GetPlayers()) do
	      TriggerClientEvent('co_notify:client:SendNotifys', i, { app = "yellow" , title = "@"..maven_datastore[1].username, content = message , source = i })
	end
