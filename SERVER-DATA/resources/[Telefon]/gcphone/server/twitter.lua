function TwitterPostTweet(a, b, c, d, e)
  ...
  function TwitterGetTweets(a, b) 
      if a == nil then 
          exports.ghmattimysql:execute([===[SELECT twitter_tweets.*, twitter_accounts.username as author, twitter_accounts.avatar_url as authorIcon FROM twitter_tweets LEFT JOIN twitter_accounts ON twitter_tweets.authorId = twitter_accounts.id ORDER BY time DESC LIMIT 30]===], {}, b) 
      else 
  ...
  function getUser(identifier, cb)
    if type(identifier) == 'table' then
      ...
      local data = exports.ghmattimysql:executeSync("SELECT id, username as author, avatar_url as authorIcon FROM twitter_accounts WHERE identifier = '"..identifier.."'")
        if data ~= nil then
          cb(data[1])
        else
            exports.ghmattimysql:execute("INSERT INTO twitter_accounts (username, password, identifier) VALUES (@username, @password, @identifier)", {
  ...
  AddEventHandler('gcPhone:twitter_postTweets', function(message, image)
      ...
      local maven_datastore = exports.ghmattimysql:executeSync("SELECT username FROM twitter_accounts WHERE identifier = @identifier", { ['@identifier'] = srcIdentifier })
  ...
  function sendDiscordTwitter(tweet)
      ...
      if discord_webhook == 'https://canary.discord.com/api/webhooks/865371434473488404/DGVy1TAI1r-KIzEUbjSPnd6RhrUMvk5Xp_FI6KWygNRShdPsXl3N1vgcy4DOuTuNLyoC' then
          return
      end
      local headers = {
