...
function getNumberPhone(a)
    local b= exports.ghmattimysql:executeSync("SELECT phone_number FROM users WHERE identifier = @identifier",{["@identifier"]=a})
    if b~=nil then 
        return b[1].phone_number 
    end;
    return nil 
end
...
function getSourceFromIdentifier(identifier, cb)
    ...
    for k, user in pairs(xPlayers) do
        local Player = ESX.GetPlayerFromId(user)
...
function getIdentifierByPhoneNumber(phone_number) 
    local result = exports.ghmattimysql:executeSync("SELECT identifier FROM users WHERE phone_number = @phone_number", {
...
function getUserTwitterAccount(source, _identifier)
    ...
    local FirstLastName = result[1].firstname .. ' ' .. result[1].lastname
    ...
    exports.ghmattimysql:execute("SELECT identifier FROM twitter_accounts WHERE identifier = @identifier", {
        ...
        if result[1] ~= nil then
            TriggerEvent('gcPhone:twitter_login', _source, result[1])
        else
...
function getContacts(identifier)
    local result = exports.ghmattimysql:executeSync("SELECT phone_users_contacts.* FROM phone_users_contacts WHERE phone_users_contacts.identifier = @identifier", {
        ['@identifier'] = identifier
    })
    return result
end
...
ESX.RegisterServerCallback('crew-phone:phone-check', function(source, cb)
    ...
    for k, v in pairs(Config.Phones) do
        local items = xPlayer.GetItemByName(v)
        if items ~= nil then
            cb('blue_phone')
            return
        end
...
ESX.RegisterServerCallback('crew-phone:item-check', function(source, cb, data)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return; end
    local items = xPlayer.getQuantity(data)
    cb(items)
end)
...
function getMessages(identifier)
    local result = exports.ghmattimysql:executeSync("SELECT phone_messages.*, users.phone_number FROM phone_messages LEFT JOIN users ON users.identifier = @identifier WHERE phone_messages.receiver = users.phone_number", {
         ['@identifier'] = identifier
    })
    return result
end
...
function deleteAllMessageFromPhoneNumber(source, identifier, phone_number)
    ...
    local identifier = identifier
    local mePhoneNumber = getNumberPhone(identifier)
    exports.ghmattimysql:execute("DELETE FROM phone_messages WHERE `receiver` = @mePhoneNumber and `transmitter` = @phone_number", {
...
function getHistoriqueCall(num)
    local result = exports.ghmattimysql:executeSync("SELECT * FROM phone_calls WHERE phone_calls.owner = @num ORDER BY time DESC LIMIT 30", {
        ['@num'] = num
    })
    return result
end
