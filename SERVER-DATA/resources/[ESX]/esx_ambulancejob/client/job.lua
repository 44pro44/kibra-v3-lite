...
function OpenAmbulanceActionsMenu()
	...
	if true and ESX.PlayerData.job.grade_name == 'boss' then
		table.insert(elements, {label = "Patron Menüsü", value = 'boss_actions'})
	end
...
function OpenMobileAmbulanceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_ambulance_actions', {
		...
		align    = 'top-left',
		elements = {
			{label = "EMS Menüsü", value = 'citizen_interaction'}
	}}, function(data, menu)
		if data.current.value == 'citizen_interaction' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				align    = 'top-left',
				elements = {
					{label = "Canlandır", value = 'revive'},
					{label = "Küçük Yaralar İçin", value = 'small'},
					{label = "Büyük Yaralar İçin", value = 'big'},
						...
						ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									isBusy = true
									ESX.Notify("Bekleyin!","Kişiyi iyileştiriyorsunuz!",3000,"info")
									...
									ESX.Notify("Başarılı",GetPlayerName(closestPlayer)..' adlı kişiyi iyileştirdiniz!',3000,"error")
					
									isBusy = false
								else
									ESX.Notify("Başarısız","Kişi Gayet Sağlıklı",3000,"error")
								end
							else
								ESX.Notify("Bulunamadı",'Bandajınız Yok',3000,"error")
							end
						end, 'bandage')
						...
						ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									ESX.Notify("Başarılı",GetPlayerName(closestPlayer)..' adlı kişiyi iyileştirdiniz!',3000,"success")
									isBusy = false
								else
									ESX.Notify("Başarısız","Kişi Gayet Sağlıklı",3000,"error")
								end
							else
								ESX.Notify("Bulunmadı",'Medikitiniz Yok',3000,"error")
							end
						end, 'medikit')
	...
	ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
		if quantity > 0 then
			...
			if IsPedDeadOrDying(closestPlayerPed, 1) then
				...
				ESX.Notify("Bekleyin!","Canlandırma Sürüyor!",3000,"warning")
...
AddEventHandler('esx_ambulancejob:hasEnteredMarker', function(hospital, part, partNum)
	...
	elseif part == 'Vehicles' then
		CurrentAction = part
		CurrentActionMsg = "~r~E~w~ - Araç Garajı"
		CurrentActionData = {hospital = hospital, partNum = partNum}
	elseif part == 'Helicopters' then
		CurrentAction = part
		CurrentActionMsg = "~r~E~w~ - Helikopter Garajı"
		CurrentActionData = {hospital = hospital, partNum = partNum}
...
AddEventHandler('esx_ambulancejob:heal', function(healType, quiet)
	...
	if not quiet then
		ESX.Notify("Başarılı", "Başarıyla iyileştirildi!",3000,"success")
	end
end)
	...
	if isOnDuty then
		...
		for playerId,status in pairs(deadPlayers) do
			if status == 'distress' then
				...
				AddTextComponentSubstringPlayerName("Ölü Biri")
