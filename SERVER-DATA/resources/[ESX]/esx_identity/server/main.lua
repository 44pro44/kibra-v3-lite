...
function getIdentity(source, callback)
	...
	exports.ghmattimysql:execute('SELECT identifier, firstname, lastname, dateofbirth, sex, height FROM `users` WHERE `identifier` = @identifier', {
...
function setIdentity(identifier, data, callback)
	exports.ghmattimysql:execute('UPDATE `users` SET `firstname` = @firstname, `lastname` = @lastname, `dateofbirth` = @dateofbirth, `sex` = @sex, `height` = @height WHERE identifier = @identifier', {
...
AddEventHandler('esx_identity:setIdentity', function(data, myIdentifiers)
	...
	setIdentity(myIdentifiers.steamid, data, function(callback)
		if callback then
			...
			TriggerClientEvent('okokNotify:Alert', source, "Başarısız!", "Karakteriniz oluşturulamadı!", 3000, 'error')
