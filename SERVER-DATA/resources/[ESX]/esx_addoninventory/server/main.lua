...
exports.ghmattimysql.ready(function()
	local items = exports.ghmattimysql:executeSync('SELECT * FROM items')

	for i=1, #items, 1 do
		Items[items[i].name] = items[i].label
	end

	local result = exports.ghmattimysql:executeSync('SELECT * FROM addon_inventory')
	...
	for i=1, #result, 1 do
		...
		local result2 = exports.ghmattimysql:executeSync('SELECT * FROM addon_inventory_items WHERE inventory_name = @inventory_name', {
			['@inventory_name'] = name
		})
