...
exports.ghmattimysql:ready(function()
	local result = exports.ghmattimysql:executeSync('SELECT * FROM jobs', {})
	...
	local result2 = exports.ghmattimysql:executeSync('SELECT * FROM job_grades', {})
	...
	if xPlayer.job.name == society.name then
		TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
			if amount > 0 and account.money >= amount then
				...
				TriggerClientEvent('okokNotify:Alert', xPlayer.source, "Para Çektiniz", "Şirket hesabınızdan <b>$"..amount..'</b> çektiniz', 3000, 'success')
			else
				TriggerClientEvent('okokNotify:Alert', xPlayer.source, "Başarısız", 'Geçersiz Miktar! Şirket Hesabınızda bu kadar para yok!', 3000, 'error')
			end
		end)
	else
	...
	if xPlayer.job.name == society.name then
		if amount > 0 and xPlayer.getMoney() >= amount then
			TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
				...
				TriggerClientEvent('okokNotify:Alert', xPlayer.source, "Para Yatırdınız", "Şirket hesabınızdan <b>$"..amount..'</b> yatırdınız', 3000, 'success')
				account.addMoney(amount)
			end)
		else
			TriggerClientEvent('okokNotify:Alert', xPlayer.source, "Başarısız", 'Geçersiz Miktar! Üzerinizde kadar para yok!', 3000, 'error')
		end
	else
...
-- 			exports.ghmattimysql:execute('INSERT INTO society_moneywash (identifier, society, amount) VALUES (@identifier, @society, @amount)', {
...
ESX.RegisterServerCallback('esx_society:getEmployees', function(source, cb, society)
	...
	local xPlayers = ESX.GetPlayers('job', society)
...
ESX.RegisterServerCallback('esx_society:setJob', function(source, cb, identifier, job, grade, type)
	...
	if isBoss then
		...
		if xTarget then
			...
			if type == 'hire' then
				TriggerClientEvent('okokNotify:Alert', xTarget.source, "Hayırlı Olsun", job..' Mesleğinde işe başladın!', 3000, 'success')
			elseif type == 'promote' then
				TriggerClientEvent('okokNotify:Alert', xTarget.source, "Hayırlı Olsun", "İşinde rütben yükseldi!", 3000, 'success')
			elseif type == 'fire' then
				TriggerClientEvent('okokNotify:Alert', xTarget.source, "Kovuldun!", xTarget.getJob().label..' tarafından işten kovuldun', 3000, 'info')
			end
...
ESX.RegisterServerCallback('esx_society:setJobSalary', function(source, cb, job, grade, salary)
	...
	if xPlayer.job.name == job and xPlayer.job.grade_name == 'boss' then
		if salary <= Config.MaxSalary then
			exports.ghmattimysql:execute('UPDATE job_grades SET salary = @salary WHERE job_name = @job_name AND grade = @grade', {
				['@salary']   = salary,
				['@job_name'] = job,
				['@grade']    = grade
			}, function(rowsChanged)
				...
				local xPlayers = ESX.GetPlayers('job', job)
...
-- 				TriggerClientEvent('okokNotify:Alert', xPlayer.source, "Kovuldun!", xTarget.getJob().label..' tarafından işten kovuldun', 3000, 'info')
-- 			end
