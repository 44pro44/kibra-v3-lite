	...
	ESX.TriggerServerCallback('esx_society:isBoss', function(isBoss)
		if isBoss then
			...
			if options.withdraw then
				table.insert(elements, {label = "Şirketinizden Para Çekin", value = 'withdraw_society_money'})
			end

			if options.deposit then
				table.insert(elements, {label = "Şirket Hesabına Para Yatır", value = 'deposit_money'})
			end
			...
			if options.employees then
				table.insert(elements, {label = "Personel Yönetimi", value = 'manage_employees'})
			end

			if options.grades then
				table.insert(elements, {label = "Maaş Yönetimi", value = 'manage_grades'})
			end
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'boss_actions_' .. society, {
				...
				elseif data.current.value == 'deposit_money' then
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'deposit_money_amount_' .. society, {
						title = "Yatırılacak Para Miktarı"
					}, function(data2, menu2)
						local amount = tonumber(data2.value)
...
function OpenManageEmployeesMenu(society)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_employees_' .. society, {
		...
		align    = 'top-left',
		elements = {
			{label = "Personel Listesi", value = 'employee_list'},
			{label = "İşe Alım Menüsü", value = 'recruit'}
	}}, function(data, menu)
	...
	ESX.TriggerServerCallback('esx_society:getEmployees', function(employees)
		...
		ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'employee_list_' .. society, elements, function(data, menu)
			...
			elseif data.value == 'fire' then
				ESX.Notify("Başarılı İşlem",employee.name..' isimli personelizin rütbesini düşürdünüz',3000,"error")
	...
	ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_' .. society, {
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_confirm_' .. society, {
				title    = data.current.name..' adlı kişiyi işe almak istiyormusun ?',
				align    = 'top-left',
				elements = {
				...
				if data2.current.value == 'yes' then
					ESX.Notift("Başarılı",data.current.name..' adlı kişiyi işe aldınız! Hayırlısı Olsun!',3000,"success")
...
function OpenPromoteMenu(society, employee)
	ESX.TriggerServerCallback('esx_society:getJob', function(job)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'promote_employee_' .. society, {
			title    = employee.name..' adlı personelin rütbesini yükselt',
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
			menu.close()
			ESX.Notify("Başarılı İşlem",employee.name..' isimli personelizin rütbesini '..data.current.label..'" e yükselttiniz',3000,"success")
...
function OpenManageGradesMenu(society)
	ESX.TriggerServerCallback('esx_society:getJob', function(job)
		...
		for i=1, #job.grades, 1 do
			...
			table.insert(elements, {
				label = gradeLabel..'- $'..job.grades[i].salary,
				value = job.grades[i].grade
			})
		end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_grades_' .. society, {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'manage_grades_amount_' .. society, {
				...
				elseif amount > Config.MaxSalary then
					ESX.Notify("Geçersiz!","Maksimum Değer Girdiniz",3000,"error")
				else
					menu2.close()
