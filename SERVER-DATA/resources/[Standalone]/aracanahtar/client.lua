...
AddEventHandler("tq-vehiclekey:client:arabayikilitlememlazim", function(plaka)
	...
	vehicles = ESX.Game.GetVehiclesInArea(pCoords, 20)
		if #vehicles == 0 then
			ESX.Notify("Bulunamadı","Yakınlarda Araç Yok!",3000,"error")
				...
				local plate = ESX.Math.Trim(GetVehicleNumberPlateText(vehicles[i]))
					if plaka == plate then
						...
						if lock == 1 or lock == 0 then
							...
							ESX.Notify("Başarılı","Araç Kilitlendi",3000,"error")
							PlayVehicleDoorCloseSound(vehicles[i], 1)
						...
						elseif lock == 2 then
							...
							PlayVehicleDoorOpenSound(vehicles[i], 0)
							ESX.Notify("Başarılı","Araç Kilidini Açtınız",3000,"success")
							if not IsPedInAnyVehicle(PlayerPedId(), true) then
								TaskPlayAnim(PlayerPedId(), animdict, "exit", 8.0, 8.0, -1, 48, 1, false, false, false)
							end
							SetVehicleLights(vehicles[i], 2)
						...
						--ESX.Notify("Geçersiz","Geçersiz Anahtar Kullandınız",3000,"warning")
					end
				end
			end
		end
...
Citizen.CreateThread(function()
      local blip = AddBlipForCoord(Config.Anahtar.x, Config.Anahtar.y, Config.Anahtar.z)
      ...
      AddTextComponentString('Anahtarcı')
      EndTextCommandSetBlipName(blip)
    
end)

exports("IsHasKey", function(plate)
	local retval = nil
	if plate == nil then  
		plate = GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), false))
	end
	ESX.TriggerServerCallback('aracanahtar:isVehicleOwner', function(owner)
		retval = owner 
	end, plate)
	while retval == nil do 
		Wait(0)
