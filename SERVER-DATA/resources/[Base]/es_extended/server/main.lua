		...
		if ESX.GetPlayerFromIdentifier(identifier) then
			...
			exports.ghmattimysql:scalar('SELECT 1 FROM users WHERE identifier = @identifier', {
				...
				if result then
					...
					exports.ghmattimysql:execute('INSERT INTO users (accounts, identifier) VALUES (@accounts, @identifier)', {
...
function loadESXPlayer(identifier, playerId)
	...
	table.insert(tasks, function(cb)
		exports.ghmattimysql:execute('SELECT accounts, job, job_grade, `group`, loadout, position, inventory FROM users WHERE identifier = @identifier', {
			...
			if result[1].position and result[1].position ~= '' then
				...
				userData.coords = {x = 101.0538, y = -1713.75, z = 30.112, heading = 46.09}
			end
	...
	Async.parallel(tasks, function(results)
		...
		xPlayer.triggerEvent('esx:playerLoaded', {
			...
			inventory = xPlayer.getInventory2(),
			job = xPlayer.getJob(),
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			if targetXPlayer.canCarryItem(itemName, itemCount) then
				...
				TriggerClientEvent('inventory:client:ItemBox', tonumber(args[1]), ESX.GetItems()[itemName], "add", itemCount)
			else
	...
	elseif type == 'item_account' then
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			TriggerClientEvent('inventory:client:ItemBox', tonumber(args[1]), ESX.GetItems()['cash'], "add", itemCount)
		else
	...
	if type == 'item_standard' then
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > xItem.count or xItem.count < 1) then
				...
				TriggerClientEvent('inventory:client:ItemBox', tonumber(args[1]), ESX.GetItems()[itemName], "remove", itemCount)
			end
		end
	elseif type == 'item_account' then
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > account.money or account.money < 1) then
				...
				TriggerClientEvent('inventory:client:ItemBox', tonumber(args[1]), ESX.GetItems()['cash'], "remove", itemCount)
			end
		end
	elseif type == 'item_weapon' then
