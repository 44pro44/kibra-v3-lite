...
ESX.Notify = function(title,msg,Time,type)
	exports['okokNotify']:Alert(title, msg, Time, type)
end

ESX.Bildirim = function(tip,msg)
	exports['mythic_notify']:SendAlert(tip, msg)
	end
...
RegisterNetEvent('esx:bildirim')
AddEventHandler('esx:bildirim', function(tip,msg)
	ESX.Bildirim(tip,msg)
end)
...
local bussy = false
ESX.Progressbar = function(name, label, duration, useWhileDead, canCancel, disableControls, animation, prop, propTwo, onFinish, onCancel)
    if not bussy then
        bussy = true
        ...
        exports.mythic_notify:SendAlert("error", "Zaten Birşey Yapıyorsun")
        onCancel()
    end
end